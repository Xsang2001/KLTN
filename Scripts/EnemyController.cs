...
public class EnemyController : MonoBehaviour
{
    ...
    private IEnumerator UpdateAI()
    {
        while (true)
        {
            ...
            else if (m_CurrenState == State.Walk)
            {
                float distance = Vector2.Distance(m_StartPosition, transform.position);
                if (distance > m_WalkDistance || CheckWallAnfGround())
                {
            ...
            else if (m_CurrenState == State.Hit)
            {
                yield return new WaitForSeconds(0.5f);
                m_GetHit = false;
                SetState(State.Run);
            }
            else if (m_CurrenState == State.Run)
            {
                float distance = Vector2.Distance(m_StartPosition, transform.position);
                if (distance > m_WalkDistance || CheckWallAnfGround())
                {
    ...
    private bool CheckWallAnfGround()
    {
        bool hitWall = false;
        Vector3 fromPos = m_CastWallPoint.position;
        Vector3 toPos = fromPos;
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        {
            Section4.AudioManager.Instance.PlaySFX_EnemyGetHit();
            m_Hp -= 1;
            if(m_Hp<=0)
            {
                Destroy(gameObject);
                GamePlayManager.Instance.onEnemyDied();
                return;
            }    
            m_GetHit = true;
            SetState(State.Hit);
